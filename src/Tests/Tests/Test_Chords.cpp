...
TEST_CASE("Chord (per file)", "[ChordsClass]")
{
    ...
    int iBufferLength = 132300;

    CVector::alloc(pfInput, iBufferLength);
    CVector::alloc(peResult, CChordIf::kNumChords);

    SECTION("Api")
    {
        CHECK(Error_t::kFunctionInvalidArgsError == CChordIf::create(pCInstance, 0, fSampleRate));
        CHECK(Error_t::kFunctionInvalidArgsError == CChordIf::create(pCInstance, iBufferLength, 0));

        CHECK(Error_t::kNoError == CChordIf::create(pCInstance, iBufferLength, fSampleRate));
